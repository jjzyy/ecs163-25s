d3.csv("data/pokemon_alopez247.csv").then(data => {
  ...
  // Bar chart: Count of Pokemon by Type 1
...
function drawScatterPlot(data) {
    ...
    // Create a color scale based on Type1
    const colorScale = d3.scaleOrdinal(d3.schemeCategory10)
                         .domain([...new Set(data.map(d => d.Type1))]);
  
    // Draw the scatter plot with color encoding by Type1
    ...
    // Legend (top 5 Type1 categories)
    const legend = svg.append("g")
                      .attr("transform", `translate(${width - 120}, 30)`);
  
    const types = colorScale.domain().slice(0, 5); // show top 5 only
  ...
  // Parallel Coordinates Plot: HP, Attack, Defense, Speed
  function drawParallelCoords(data) {
    const svg = d3.select("#parallelCoords svg"),
          width = +svg.attr("width"),
          height = +svg.attr("height"),
